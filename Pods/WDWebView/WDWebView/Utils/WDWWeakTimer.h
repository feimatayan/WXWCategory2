...
/** copy of msweaktimer 1.1.0
 * */
@interface WDWWeakTimer : NSObject
 ...
 * @param repeats if `YES`, `selector` will be invoked on `target` until the `WDWWeakTimer` object is deallocated or until you call `invalidate`. If `NO`, it will only be invoked once.
...
/**
 * Creates an `WDWWeakTimer` object and schedules it to start ticking inmediately.
 */
 ...
 * @note `WDWWeakTimer` won't invoke the `selector` on `target` again after calling this method.
