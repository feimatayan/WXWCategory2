...
#import "GLImageEditorViewController.h"
#import "GLImageEditorFrameView.h"
...
@interface GLImageEditorViewController ()
...
@implementation GLImageEditorViewController
...
- (void)commonInit
{
    ...
    GLImageEditorFrameView *frameView = [[GLImageEditorFrameView alloc] initWithFrame:[UIScreen mainScreen].bounds];
...
- (void)viewDidLoad
{
    ...
    okButton.frame = CGRectMake(self.view.frame.size.width - 60, self.view.frame.size.height - 25, 50, 20);
    ...
    cancelButton.frame =  CGRectMake(10, self.view.frame.size.height - 25, 50, 20);
...
- (IBAction)handlePan:(UIPanGestureRecognizer*)recognizer
{
    if([self handleGestureState:recognizer.state]) {
        ...
        //如果没有缩放或不支持缩放，就不要在x坐标上移动了
        if(self.scale == 1 || !self.scaleEnabled)
        {
            //判断图片尺寸比例和裁剪区域比例的大小比较
            CGFloat aspect = self.sourceImage.size.height/self.sourceImage.size.width;
            CGFloat cropAspect = self.cropRect.size.height / self.cropRect.size.width;
