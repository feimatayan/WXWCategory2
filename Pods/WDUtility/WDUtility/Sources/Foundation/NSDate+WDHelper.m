//
// Created by 陈岳燊 on 16/10/13.
//

#import "NSDate+WDHelper.h"

@implementation NSDate (WDHelper)
...
#pragma mark - Format Comment Date

- (NSString *)formatCommentDate {
    NSString *hourAndMinute = [NSString stringWithFormat:@"%@:%@", [self formatHourOrMinute:self.hour], [self formatHourOrMinute:self.minute]];
    if ([self isThisYear]) {
        return [NSString stringWithFormat:@"%ld月%ld日 %@", (long)self.month, (long)self.day, hourAndMinute];
    } else {
        return [NSString stringWithFormat:@"%ld年%ld月%ld日 %@", (long)self.year, (long)self.month, (long)self.day, hourAndMinute];
    }
}

- (NSString *)formatInteractMsgDate {
    NSString *hourAndMinute = [NSString stringWithFormat:@"%@:%@", [self formatHourOrMinute:self.hour], [self formatHourOrMinute:self.minute]];
    if ([self isToday]) {
...
#pragma mark - Private
- (NSString *)formatHourOrMinute:(NSInteger)hourOrMinute {
    NSString *formatHourOrMinute;
    if (hourOrMinute < 10) {
        formatHourOrMinute = [NSString stringWithFormat:@"0%ld", (long)hourOrMinute];
    } else {
        formatHourOrMinute = [NSString stringWithFormat:@"%ld", (long)hourOrMinute];
    }
    return formatHourOrMinute;
}
